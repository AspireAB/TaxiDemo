    ...
    public class TaxiActor : ReceiveActor
    {
        ...
        public void Driving()
        {
            ...
            Receive<Taxi.Position>(p =>
            {
                ...
                {
                    _presenter.Tell(new Taxi.PositionBearing(p.Longitude, p.Latitude, Bearing(), GpsStatus.Parked, _id,
                        _source));
                    Become(Parked);
                }
                else
                {
                    _presenter.Tell(new Taxi.PositionBearing(p.Longitude, p.Latitude, Bearing(), GpsStatus.Active, _id,
                        _source));
                }
            });
        }
        ...
        {
            Receive<Taxi.Idle>(_ => { Become(Disconnected); });
        }
